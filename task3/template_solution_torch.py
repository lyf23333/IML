...
def get_data(file, train=True):
    ...
    for t in triplets:
        ...
        if X is None:
            X = torch.cat((torch.tensor(emb[0]), torch.tensor(emb[1]), torch.tensor(emb[2]))).unsqueeze(0).to(device)
            y = torch.tensor([[1]]).to(device)
        else:
            X = torch.cat((X, torch.cat((torch.tensor(emb[0]), torch.tensor(emb[1]), torch.tensor(emb[2]))).unsqueeze(0).to(device)), dim=0)
            y = torch.cat((y, torch.tensor([[1]]).to(device)), dim=0)
        count += 1
...
if __name__ == '__main__':
    ...
    X, y = get_data(TRAIN_TRIPLETS_AUG)
