...
def generate_embeddings():
    ...
    with torch.no_grad():
        for j in range(num_groups):
            ...
            np.save(f'task3/dataset/embeddings_{j}.npy', embeddings)
...
def get_data(file, train=True):
    ...
    y = []
    for j in range(num_groups):
        embeddings = np.load(f'task3/dataset/embeddings_{j}.npy')
